...
- 🚀 iOS **build #18 (`763793bd`) is VALID in App Store Connect / TestFlight (2026-07-02 ~12:20)** — the full chain worked (build → auto-submit delivered → Apple processed VALID). Build #17 uploaded but was REJECTED in Apple processing with ITMS-90683 (missing `NSPhotoLibraryUsageDescription` — a bundled SDK references the Photo Library API); fixed by adding NSPhotoLibrary + NSPhotoLibraryAdd purpose strings to ios.infoPlist. NOTE: build #17's overnight auto-submit silently failed; #17 was pushed via manual `eas submit --id` (which is how we saw the ITMS rejection). #18's auto-submit worked.
- ⏳ Next: install on device via TestFlight (add Troy as internal tester if not auto-visible). Fill App Store Connect export-compliance + test notes (Sasha's checklist). Then set up Play submission.
