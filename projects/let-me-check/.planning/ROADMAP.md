...
- [x] 09-03-PLAN.md — Reconnect screens: persist notification prefs + preferred cities + real profile stats (wave 2)
- [x] 09-04-PLAN.md — [BLOCKING] db push 0017 + live pgTAP + regen types + tsc (wave 3)
...
Plans:
- [x] 10-01-PLAN.md — install expo-notifications/expo-device + plugin; migration 0018 device_push_tokens + RLS + dispatching push trigger + RED pgTAP (Wave 1)
- [x] 10-02-PLAN.md — send-push Edge Function (recipient resolution, prefs, Expo batch, DeviceNotRegistered cleanup, never-throws) + Deno tests (Wave 2)
- [x] 10-03-PLAN.md — client push.ts (register/upsert/delete) + auth.ts wiring + sign-out cleanup + Scout job-nearby toggle + vitest (Wave 2)
- [x] 10-04-PLAN.md — mux-webhook step 8c: fire-and-forget seeker video-ready push on delivered (Wave 3)
- [x] 10-05-PLAN.md — BLOCKING deploy: db push 0018 + GUCs, deploy send-push + redeploy mux-webhook (--no-verify-jwt), regen types, full suite (Wave 4)
...
- [x] 11-02-PLAN.md — hide growth screens (membership/invite entry points) + wire search location button + remove SHOW_BLUR_TEST + WF badges (Wave 2)
- [x] 11-03-PLAN.md — Delete-Account entry points (both profiles) + deleteMyAccount helper + privacy/terms/support links + remove unused photo permission (Wave 3)
- [x] 11-04-PLAN.md — [BLOCKING] live: db push 0021 + GREEN pgTAP + deploy delete-account (verify_jwt=true) + reset dispatch_timeout_s=300 + regen types + tsc (Wave 4)
- [x] 11-05-PLAN.md — HUMAN doc: APPLE-SUBMISSION-CHECKLIST + privacy-policy draft + demo-account seed script (Wave 5)
