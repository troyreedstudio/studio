...
import '../controller/newsfeed_profile_controller.dart';

class NewsfeedProfilePage extends StatefulWidget {
  const NewsfeedProfilePage({super.key, this.userID});

  final String? userID;

  @override
  State<NewsfeedProfilePage> createState() => _NewsfeedProfilePageState();
}

class _NewsfeedProfilePageState extends State<NewsfeedProfilePage> {
  late final NewsFeedProfileController controller;
  final newsFeedUserModel = NewsFeedUserModel();
  late final hasBio = (newsFeedUserModel.userProfile?.bio?.isNotEmpty ?? false);

  @override
  void initState() {
    super.initState();
    print("🎯 NewsfeedProfilePage initState - userID: ${widget.userID}");

    // Create a fresh controller instance
    controller = NewsFeedProfileController();
    Get.put(controller, tag: widget.userID);

    final id = widget.userID;
    if (id != null && id.isNotEmpty) {
      print("🎯 Calling controller.setUser with: $id");
      ...
      AppSnackbar.show(message: "No user id passed", isSuccess: false);
    }
  }

  @override
  void dispose() {
    print("🗑️ Disposing controller for userID: ${widget.userID}");
    controller.scrollCtrl.dispose();
    Get.delete<NewsFeedProfileController>(tag: widget.userID);
    super.dispose();
  }
      ...
      body: CustomScrollView(
        ...
        slivers: [
          ...
          SliverAppBar(
            ...
            title: Obx(() {
              return Text(
                controller.newsFeedUserProfile.value?.userProfile?.fullName ??
                    "no name",
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                smallerText(
                                  text:
                                      "@${controller.newsFeedUserProfile.value?.userProfile?.username ?? "no username"}",
                            ...
                            child: Row(
                              children: [
                                ...
                                Obx(() {
                                  ...
                                  print(
                                    "🔵 Button rebuilt - isFollowing: $isFollowing, isLoading: $isLoading",
                                  );
                                  ...
                                  return ElevatedButton.icon(
                                    onPressed: isLoading
                                        ...
                                        : () {
                                            print(
                                              "🔴 BUTTON CLICKED! isFollowing: $isFollowing, isLoading: $isLoading",
                                            );
                                ...
                                IconButton(
                                  onPressed: () async {
                                    ...
                                    // Try to fix userId first if not available
                                    final localService = LocalService();
                                    String? myUserId = await localService
                                        .getValue<String>(PreferenceKey.userId);

                                    if (myUserId == null) {
                                      print(
                                        '⚠️ User ID not found, attempting to fetch...',
                                      );

                                      // Import করতে হবে: import 'package:pineapple/feature/chat_tab/helper/user_id_fixer.dart';
                                      myUserId =
                                          await UserIdFixer.ensureUserId();
                                    ...
                                    // Get receiver info with proper fallbacks
                                    final profileData =
                                        controller.newsFeedUserProfile.value;

                                    final receiverId =
                                        profileData?.userProfile?.id ??
                                        widget.userID;

                                    // Try to get name from profile data, otherwise fetch from user info
                                    ...
                                    if (profileData?.userProfile != null) {
                                      ...
                                      // Profile data not loaded yet, get from controller if available
                                      print(
                                        '⚠️ Profile data not loaded, attempting to load...',
                                      );

                                      // Trigger profile fetch if not done
                                      if (receiverId != null &&
                                          receiverId.isNotEmpty) {
                                        await controller.getNewsFeedProfile(
                                          receiverId,
                                        );

                                        // Try again after fetch
                                        final updatedProfile = controller
                                            .newsFeedUserProfile
                                            .value;
                                    ...
                                    print('✅ Opening chat detail page...');
                                    ...
                                    print('↩️ Returned from chat detail page');
                                  },
                                  icon: FaIcon(
                    ...
                    Padding(
                      ...
                      child: _StatsRow(
                        items: [
                          _StatItem(
                            count:
                                "${controller.newsFeedUserProfile.value?.post ?? '0'}",
                            label: 'Posts',
                          ),
                          _StatItem(
                            count:
                                "${controller.newsFeedUserProfile.value?.follower ?? '0'}",
                            label: 'Followers',
                          ),
                          _StatItem(
                            count:
                                "${controller.newsFeedUserProfile.value?.following ?? '0'}",
                            label: 'Followings',
                          ),
                        ],
                      ),
...
Widget _postCard(
  ...
  NewsFeedProfileController controller,
) {
  return Container(
